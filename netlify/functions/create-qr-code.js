  ...
  try {
    ...
    const response = await fetch("https://api.yookassa.ru/v3/payments", {
      ...
      body: JSON.stringify({
        ...
        description: "Оплата через Юкасса",
        payment_method_data: {
          type: "yoomoney", // Используем YooMoney для получения QR-кода
        },
      }),
    });
    ...
    return {
      statusCode: 200,
      body: JSON.stringify({ qrCodeUrl: data.confirmation.confirmation_url }), // Возвращаем ссылку на QR-код
    };
  } catch (error) {
    return {
      statusCode: 500,
